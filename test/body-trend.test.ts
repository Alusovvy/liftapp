...
describe("neutral body trends", () => {
  test("requires three observations spanning at least fourteen days", () => {
    const data = dataWithMetrics([
      ...
      { id: "two", date: "2026-07-08", weightKg: 79.8, bodyFatPercent: null, note: "", recordedAt: "" },
    ]);
  ...
  test("uses the median consecutive weekly change and neutral direction language", () => {
    const data = dataWithMetrics([
      ...
      { id: "two", date: "2026-07-08", weightKg: 79.5, bodyFatPercent: 19.8, note: "", recordedAt: "" },
      { id: "three", date: "2026-07-15", weightKg: 79, bodyFatPercent: 19.6, note: "", recordedAt: "" },
    ]);
  ...
  test("classifies small median changes as stable without value judgment", () => {
    const data = dataWithMetrics([
      { id: "one", date: "2026-07-01", weightKg: 80, bodyFatPercent: null, note: "", recordedAt: "" },
      { id: "two", date: "2026-07-08", weightKg: 80.05, bodyFatPercent: null, note: "", recordedAt: "" },
      { id: "three", date: "2026-07-15", weightKg: 80.04, bodyFatPercent: null, note: "", recordedAt: "" },
    ]);
  ...
  test("filters to the selected window and flags likely entry errors without deleting them", () => {
    const data = dataWithMetrics([
      ...
      { id: "three", date: "2026-07-20", weightKg: 80, bodyFatPercent: 18, note: "", recordedAt: "" },
    ]);
    const weight = buildBodyTrend(data, "weightKg", 30);
    ...
    assert.equal(weight.points.some((point) => point.id === "two"), true);
  });

  test("ignores measurements where the selected metric was not recorded", () => {
    const data = dataWithMetrics([
      { id: "fat-only", date: "2026-07-01", weightKg: null, bodyFatPercent: 20, note: "", recordedAt: "" },
    ]);
