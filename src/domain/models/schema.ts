...
export const EffortSourceSchema = z
  .enum(["manual", "manual-cleared", "explicit-rir", "derived-from-rpe", "missing"])
  .or(z.string());
...
export const RecoveryCheckinSchema = z
  .object({
    id: z.string().min(1),
    date: DateKeySchema,
...
export const NutritionDaySchema = z
  .object({
    id: z.string().min(1),
    date: DateKeySchema,
...
const TargetRangeSchema = z
  ...
  .refine(
    ...
    "Target maximum must be at least the minimum",
  );

export const TargetsSchema = z.object(
  Object.fromEntries(MUSCLES.map((muscle) => [muscle, TargetRangeSchema])) as Record<
    (typeof MUSCLES)[number],
    typeof TargetRangeSchema
  >,
);
...
export const LiftwiseDataSchema = z
  .object({
...
const DEFAULT_TARGETS: Record<Muscle, [number, number]> = {
  Chest: [8, 14],
  Back: [10, 16],
  Quads: [8, 14],
  Hamstrings: [8, 14],
...
// The smallest object that satisfies LiftwiseDataSchema: a plain starting
// point for a brand-new account with no history yet. Home-gym equipment
// defaults match the documented default profile (README.md, "Home-gym
// defaults"). Callers can save this as-is and adjust it afterward.
export function createDefaultLiftwiseData(name = "Athlete"): LiftwiseData {
  return LiftwiseDataSchema.parse({
    ...
    libraryPreferences: { availableOnly: false, density: "comfortable", sort: "catalog" },
    integrations: {
      garmin: { status: "not_connected", lastSyncAt: null },
      fitatu: { status: "not_connected", lastImportAt: null, lastFileName: null },
    },
