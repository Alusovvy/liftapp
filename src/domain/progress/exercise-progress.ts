...
import type { LiftwiseData, Workout, WorkoutEntry, WorkoutSet } from "../models/schema";
...
function signature(entry: WorkoutEntry, mode: MeasurementMode): string {
  return [mode, entry.loadMode ?? "unspecified-load", entry.repMode ?? "total"].join(":");
}
...
function performanceLabel(set: WorkoutSet, mode: MeasurementMode): string {
  if (mode === "duration") return secondsLabel(numeric(set.durationSeconds));
  if (mode === "distance_duration") {
    const distance = numeric(set.distanceMeters);
    const distanceLabel =
      distance >= 1000 ? `${Math.round((distance / 1000) * 100) / 100} km` : `${distance} m`;
...
function toPerformance(workout: Workout, entry: WorkoutEntry): ComparablePerformance | null {
...
function improved(current: ComparablePerformance, previous: ComparablePerformance): boolean {
  ...
  if (current.mode === "distance_duration") {
    return (
      numeric(current.distanceMeters) > numeric(previous.distanceMeters) ||
  ...
  const loadImproved = assistance ? currentLoad < previousLoad : currentLoad > previousLoad;
...
function maintainedRecommendation(mode: MeasurementMode): string {
  if (mode === "duration")
    return "Repeat the setup and add a small amount of time if form stays stable.";
  if (mode === "distance_duration")
    return "Repeat the route or setup before changing distance or pace.";
...
export function buildExerciseProgress(data: LiftwiseData): ExerciseProgressSummary[] {
  ...
  return [...exerciseIds]
    .map((exerciseId): ExerciseProgressSummary => {
      ...
      if (comparable.length === 1) {
        return {
          ...
          reason:
            "One qualified appearance establishes a baseline; a second comparable session enables a change review.",
      ...
      const hardThree =
        latestThree.length === 3 && latestThree.every((item) => item.rir !== null && item.rir <= 1);
      const noRecentImprovement =
      ...
      if (hardThree && noRecentImprovement) {
        return {
          ...
          reason:
            "Performance has not improved across three comparable hard sessions. This is a review prompt, not a diagnosis.",
          lastDate: latest.date,
          lastPerformance: latest.label,
          comparableHistory: comparable,
          excludedAppearances,
