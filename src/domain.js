...
function detectDelimiter(source) {
  ...
  candidates.forEach((delimiter) => {
    const counts = lines
      .map((line) => delimiterCount(line, delimiter))
    ...
    if (character === '"') {
      ...
      if (row.some((value) => String(value).trim() !== "")) {
        ...
        if (rows.length > maxRows)
          throw new Error(`The CSV exceeds the ${maxRows.toLocaleString()}-row safety limit.`);
      }
      row = [];
    } else {
      field += character;
...
function localizedNumber(value) {
  ...
  let input = original.replace(/[\u00A0\u202F\s']/g, "").replace(/[^\d,.\-+]/g, "");
...
function fitatuHeaderIndex(headers, field) {
  ...
  const blocked =
    /(?:^|_)(?:goal|target|limit|cel|norma|recommended|zalecane|procent|percent)(?:_|$)/;
...
function fitatuHeaderMap(row) {
  ...
  const result = Object.fromEntries(
    Object.keys(FITATU_HEADER_ALIASES).map((field) => [field, fitatuHeaderIndex(headers, field)]),
  );
...
function mostCompleteNutritionRow(rows) {
  return [...rows].sort((first, second) => {
    const completeness = (row) =>
      ["caloriesKcal", "proteinG", "carbsG", "fatG", "fiberG"].filter(
...
function parseFitatuExport(text, options = {}) {
  ...
  const today =
    fitatuDateKey(options.today) || fitatuDateKey(new Date().toISOString().slice(0, 10));
  ...
  for (let index = 0; index < Math.min(rows.length, 30); index += 1) {
    ...
    const macroCount = [candidate.proteinG, candidate.carbsG, candidate.fatG].filter(
  ...
  if (headerIndex < 0) {
    throw new Error(
      "This does not look like a Fitatu nutrition CSV. Expected a date, calories, and at least two macronutrient columns.",
    );
  }
...
function sourceIdentity(workout = {}) {
  ...
  if (workout.providerWorkoutId)
    return `${workout.source || "provider"}:${workout.providerWorkoutId}`;
  ...
  return {
    ...
    entries: (incoming.entries || []).map((entry) => ({
      ...
      sets: (entry.sets || []).map((set, index) => {
        ...
        return normalizeEffortSet(manual ? { ...set, ...manual, rirManual: true } : set, {
...
function compareSourceWorkout(existing, incoming) {
  ...
  if (before === after)
    return { status: "unchanged", existing, incoming: { ...incoming, contentFingerprint: after } };
  ...
  const updated = overlayManualEffort(existing, {
    ...incoming,
    id: existing.id,
...
function normalizedSetVolume(set = {}, loadMode = "total", repMode = "total") {
  ...
  const loadFactor =
    loadMode === "per_hand" && repMode !== "per_side"
      ? 2
      : loadMode === "assistance" || loadMode === "none"
        ? 0
        : 1;
...
function normalizeBodyMetric(metric, index = 0, today = toDateInput(new Date())) {
  const date = isValidDateKey(metric?.date) ? String(metric.date) : null;
  const weightKg = numberOrNull(metric?.weightKg ?? metric?.weight_kg ?? metric?.weight);
  const bodyFatPercent = numberOrNull(
    metric?.bodyFatPercent ?? metric?.body_fat_percent ?? metric?.bodyFat ?? metric?.body_fat,
  );
  const validWeight = weightKg !== null && weightKg >= 20 && weightKg <= 500 ? weightKg : null;
  const validBodyFat =
    bodyFatPercent !== null && bodyFatPercent >= 1 && bodyFatPercent <= 100 ? bodyFatPercent : null;
  if (!date || date > today || (validWeight === null && validBodyFat === null)) return null;
  const source = metric?.source === "garmin" ? "garmin" : "manual";
  return {
    id: String(
      metric?.id ||
        `body-${date}-${index}-${hash(`${date}-${index}-${validWeight}-${validBodyFat}`)}`,
    ),
    date,
    ...
    condition: ["morning-fasted", "morning", "evening", "other"].includes(metric?.condition)
      ? metric.condition
      : "",
    note: String(metric?.note || "").slice(0, 500),
    recordedAt:
      metric?.recordedAt && !Number.isNaN(new Date(metric.recordedAt).getTime())
        ? String(metric.recordedAt)
        : `${date}T12:00:00.000Z`,
  };
}

function normalizeBodyMetrics(metrics, today = toDateInput(new Date())) {
  const seenGarminRecords = new Set();
  return metrics
    .map((metric, index) => normalizeBodyMetric(metric, index, today))
    .filter(Boolean)
    .filter((metric) => {
...
function nutritionDayFingerprint(day) {
  return `nutrition:${hash(
    stableStringify({
      date: day.date,
      caloriesKcal: numberOrNull(day.caloriesKcal),
      proteinG: numberOrNull(day.proteinG),
      carbsG: numberOrNull(day.carbsG),
      fatG: numberOrNull(day.fatG),
      fiberG: numberOrNull(day.fiberG),
    }),
  )}`;
}

function normalizeNutritionDay(day, _index = 0, today = toDateInput(new Date())) {
  const date = isValidDateKey(day?.date) && day.date <= today ? String(day.date) : null;
  if (!date) return null;
  ...
  const normalized = {
    ...
    sourceRowCount: Math.max(
      1,
      Math.round(numberOrNull(day?.sourceRowCount ?? day?.rowCount) || 1),
    ),
    aggregation: ["items", "daily-total", "total", "meal-totals"].includes(day?.aggregation)
      ? day.aggregation
      : "items",
    importBatchId: day?.importBatchId ? String(day.importBatchId).slice(0, 160) : null,
    importedAt:
      day?.importedAt && !Number.isNaN(new Date(day.importedAt).getTime())
...
function normalizeNutritionDays(days, today = toDateInput(new Date())) {
  const byDate = new Map();
  (Array.isArray(days) ? days : [])
    .map((day, index) => normalizeNutritionDay(day, index, today))
    .filter(Boolean)
    .forEach((day) => {
      const current = byDate.get(day.date);
      if (!current || new Date(day.importedAt) >= new Date(current.importedAt))
        byDate.set(day.date, day);
    });
  return [...byDate.values()].sort((first, second) => first.date.localeCompare(second.date));
}

function normalizeRecoveryCheckin(checkin, index = 0, today = toDateInput(new Date())) {
  const date = isValidDateKey(checkin?.date) ? String(checkin.date) : null;
  const sleepHours = numberOrNull(checkin?.sleepHours);
  const energy = numberOrNull(checkin?.energy);
  const soreness = numberOrNull(checkin?.soreness);
  ...
  return {
    ...
    recordedAt:
      checkin?.recordedAt && !Number.isNaN(new Date(checkin.recordedAt).getTime())
        ? String(checkin.recordedAt)
        : `${date}T12:00:00.000Z`,
  };
}

function normalizeRecoveryCheckins(checkins, today = toDateInput(new Date())) {
  const byDate = new Map();
  checkins
    .map((checkin, index) => normalizeRecoveryCheckin(checkin, index, today))
    .filter(Boolean)
    .forEach((checkin) => {
      const current = byDate.get(checkin.date);
      if (!current || new Date(checkin.recordedAt) >= new Date(current.recordedAt))
    ...
    .map(([date, sessions]) => {
      ...
      sessions.forEach((session) =>
        (session.entries || []).forEach((entry) => {
          if (!entries.has(entry.exerciseId))
            entries.set(entry.exerciseId, { ...entry, sets: [], sourceSessionIds: [] });
          const aggregate = entries.get(entry.exerciseId);
          aggregate.sets.push(...(entry.sets || []));
          if (!aggregate.sourceSessionIds.includes(session.id))
            aggregate.sourceSessionIds.push(session.id);
        }),
      );
      return {
        date,
